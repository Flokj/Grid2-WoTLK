...
-- 3.3.5: Grid2Options.LI (custom indicator names table) does not exist, provide a fallback table
Grid2Options.LI = Grid2Options.LI or {}
...
function RDO:FixWrongInstances()
	-- 3.3.5 backport: profile subtables may not exist (fresh profile or the
	-- standalone engine disabled, so its defaults never merged) — default them.
	local profile = RDO.db.profile
