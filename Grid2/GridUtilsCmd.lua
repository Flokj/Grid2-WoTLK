...
-- 3.3.5 port: identical to BCC, plus Grid2:ProcessCommandLine() returns true
-- when a command was handled, so Grid2:OnChatCommand() in GridCore.lua can
-- forward anything else to Grid2Options.
		...
		if param then
			...
			if header then
				if flag then
					cmd = rst
				elseif rst then
					print("Grid2 Error: Too many command line arguments or specified header type not found in active layout.");
					return
				end
				if cmd==nil then
...
function Grid2:ProcessCommandLine(input)
	...
	elseif c=='namelist' or c=='nl' then
		...
		return -- not handled, let the caller forward the command elsewhere
	end
	return true
end
