...
-- 3.3.5: no Grid2.API.UnitAuraLite (UnitAura/C_UnitAuras do not exist), group scanning
-- falls back to UnitBuff(); full multi-aura support needs the new aura engine.
local Grid2 = Grid2
local bit_bor= bit.bor
local SpellIsSelfBuff = SpellIsSelfBuff or function() end -- 3.3.5: API missing
local UnitAffectingCombat = UnitAffectingCombat
local SpellGetVisibilityInfo = SpellGetVisibilityInfo -- nil on 3.3.5, guarded in ShouldDisplayBuff()
local myUnits = Grid2.roster_my_units or { player = true, pet = true, vehicle = true } -- 3.3.5: no roster_my_units
local UnitAura = (Grid2.API and Grid2.API.UnitAuraLite) or function(unit, index) return UnitBuff(unit, index) end
...
local function status_UpdateStateStrict(s, u, i, sid, nam, tex, cnt, dur, exp)
	...
	mask = bit_bor( mask, s.spells[nam] or (sid and s.spells[sid]) ) -- 3.3.5: sid may be nil
