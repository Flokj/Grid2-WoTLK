-- Status: Aoe-Heals
-- 3.3.5: COMBAT_LOG_EVENT_UNFILTERED passes (timestamp, subEvent, hideCaster,
-- srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, ...).
-- CombatLogGetCurrentEventInfo() does not exist, args are read from the event payload
-- (AceEvent passes the event name as first callback arg, it is discarded).

-- 3.3.5 backport: the standalone Grid2AoeHeals addon creates a module with the
-- same name; reuse it instead of throwing "Module already exists".
local AOEM = Grid2:GetModule( "Grid2AoeHeals", true) or Grid2:NewModule( "Grid2AoeHeals", "AceEvent-3.0")
				...
				if mine == nil or mine == (select(4,...)==playerGUID) then
					...
					if unit then
						...
						if prev~=spellName then
							status:UpdateIndicators(unit)
								timer = timer or Grid2:ScheduleRepeatingTimer(TimerEvent, timerDelay)
						end
					end
				end
			end
