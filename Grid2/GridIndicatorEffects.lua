...
-- 3.3.5 port: LibCustomGlow-1.0 does not exist on this client, the bundled
-- SpecializedLibGlow-1.0 provides the same PixelGlow/AutoCastGlow/ButtonGlow
-- functions (the key argument is optional there, so the calls are unchanged).
...
-- Blink effect
-- 3.3.5 backport: Alpha animations here have no SetFromAlpha/SetToAlpha
-- (added later), only SetChange — same pattern as GridFrame blink.
