...
local GetSpellInfo = (Grid2.API and Grid2.API.GetSpellInfo) or _G.GetSpellInfo
...
function Grid2Options:MakeStatusDebuffTypeExtraOptions(status, options, optionParams)
	...
	options.extraDebuffs = {
		...
		set = function(_, v)
			if Grid2.UnregisterDebuffTypeSpells then Grid2:UnregisterDebuffTypeSpells(status.dbx.subType, status.dbx.debuffsList, true) end
			local debuffsList = {}
			...
			if Grid2.RegisterDebuffTypeSpells then Grid2:RegisterDebuffTypeSpells(status.dbx.subType, status.dbx.debuffsList, true) end
			status:Refresh()
		end,
	}
end
