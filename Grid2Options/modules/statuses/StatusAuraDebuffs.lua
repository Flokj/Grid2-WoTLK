...
function Grid2Options:MakeStatusDebuffsFilterOptions(status, options, optionParams)
	...
	MakeCondition("filterRelevant", false,
		"Relevant Debuffs",
		"Display debuffs marked as relevant.",
		"Non-Relevant Debuffs",
		"Display debuffs marked as non-relevant."
	)
...
Grid2Options:RegisterStatusOptions("debuffs", "debuff", function(self, status, options, optionParams)
	...
	local general = self:MakeStatusDebuffsGeneralOptions(status, {}, optionParams)
	options.settings = {
		type = "group", order = 10, name = L["General"],
		args = general,
	}
	local list = {}
	self:MakeStatusAuraListOptions(status, list, optionParams)
	options.debuffslist = {
