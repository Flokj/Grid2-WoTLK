-- Grid2 Themes support (backported from Grid2 2.9.31, WoTLK adaptation).
-- Requires Grid2 core themes support (Grid2.currentTheme, Grid2:ReloadTheme(),
-- Grid2.db.profile.themes, Grid2Frame/Grid2Layout extraThemes database).
-- The file itself loads safely when core themes support is missing, the Themes
-- section simply stays hidden until the core provides the themes database.
...
-- WoTLK core uses AceDB (.db), classic Grid2 code used a .dba wrapper.
-- Support both so the file works regardless of the core database accessor.
local function GetModuleProfile(module)
	local db = module and (module.dba or module.db)
	return db and db.profile
end

local function CoreThemesEnabled()
	local profile = GetModuleProfile(Grid2Frame)
	return profile ~= nil and profile.extraThemes ~= nil
end
...
do
	...
	do
		...
		-- current class + specs (WoTLK: dual spec, Primary/Secondary)
		local count = (Grid2.GetNumSpecializations and Grid2.GetNumSpecializations()) or (GetNumTalentGroups and GetNumTalentGroups()) or 1
		if count > 1 then
	...
	Grid2Options:MakeTitleOptions( _options, L["Themes"], L["themes management"], nil, "Interface\\ICONS\\INV_Misc_Note_06" )
